...
#  iterates over all players, and all seasons of a player
for player in players:
    ...
    for season in seasons:
        ...
        # applied calculations so the x values are placed well on the court
        for value in shots:
        ...
        # create the dict with all the data
        for value in shots:
            tenderData.append({"x": x[i], "y":y[i], "made": made[i], "attempts": attempts[i], "z": 1})
            i+=1
        i=0

        #  if the data is empty, so a player hasn't played in that season, then nothing is written
        if tenderData:
            print season
